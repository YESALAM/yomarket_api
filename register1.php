<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
               ...
               $conn2 =   mysqli_connect("localhost","simptnhu","+d2n4?%KwE7!","simptnhu_yomarket","3306");
           ...
           $filename="";
                  ...
                  $sql_check = "SELECT * FROM `register2` WHERE `register_email_address` = '$register_email_address'" ;
                  $check_result = mysqli_query($conn2,$sql_check);

                  $row_cnt = mysqli_num_rows($check_result);
                  if($row_cnt > 0){
                    echo json_encode(array("server response"=>'name_exist'));
       ...
       // $rotate = imagerotate($source, $angle, 0); // if want to rotate the image
          ...
          mkdir($path, 0777, true);
           ...
           $sql = "INSERT INTO `register2`(`register_name`, `register_email_address`, `register_password`, `register_mobile_no`, `register_firm_name`, `register_profession_type`, `register_city_type`,`register_state`, `register_country`, `register_product`,`pic_name`)
           VALUES ('$register_name','$register_email_address','$register_password','$register_mobile_no','$register_firm_name','$register_profession_type','$register_city_type','$register_state','$register_country','$register_product','$imagename')";
